...
.danger{{background:#9b1c1c}} .muted{{color:#52616f}} .error{{background:#ffe9e9;border-color:#f2b8b8}} .ok{{background:#e9f7ef;border-color:#a9dfbf}}
header{{display:flex;align-items:center;justify-content:space-between;gap:10px}} code{{background:#eef2f6;padding:2px 5px;border-radius:5px}}
</style></head><body><header><h1>{esc(title)}</h1><nav><a href="/">Settings</a> · <a href="/logout">Log out</a></nav></header>{body}</body></html>""".encode()
...
<div class="card"><form method="post" action="/login"><label>Username</label><input name="username" required autocomplete="username"><label>Password</label><input name="password" type="password" required autocomplete="current-password"><button>Log in</button></form></div>"""))
        ...
        body = [msg, f'<div class="card muted">Editing <code>{esc(CONFIG_FILE)}</code>. Backups are created beside the config before every save.</div>', '<form method="post" action="/save">']
    ...
    def do_POST(self) -> None:
        if self.path == "/setup":
            ...
            if not username or len(credential) < 8:
                ...
                return
            salt, digest = hash_password(credential)
